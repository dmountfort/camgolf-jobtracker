...
export default function JobCardReport(){
 ...
 const {data,error}=await db.from("jobs").select("job_number,invoice_number,service_date,travelling_km,duration_hours,general_notes,customers(name,address,telephone,email),job_vehicles(id,model,unit_number,serial_number,amp_hours,work_performed,attention_notes,sort_order),job_parts(job_vehicle_id,description,quantity,parts(part_number))").eq("id",id).is("removed_at",null).single();if(error)throw error;
