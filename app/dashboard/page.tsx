...
type JobRow={id:string;job_number:number;invoice_number:string|null;status:Status;service_date:string;updated_at:string;customers:{name:string}|null;sites:{name:string}|null;profiles:{full_name:string}|null;job_vehicles:{count:number}[]};
...
export default function Dashboard(){
 ...
 useEffect(()=>{(async()=>{const supabase=createClient();const {data:{user}}=await supabase.auth.getUser();if(!user){window.location.href="/";return}const {data:profile}=await supabase.from("profiles").select("role").eq("id",user.id).single();if(profile?.role!=="admin"){window.location.replace("/field");return}const {data,error}=await supabase.from("jobs").select("id,job_number,invoice_number,status,service_date,updated_at,customers(name),sites(name),profiles!jobs_technician_id_fkey(full_name),job_vehicles(count)").is("removed_at",null).order("updated_at",{ascending:false});if(error)setError(error.message);else setJobs((data||[]) as unknown as JobRow[]);setLoading(false)})()},[]);
