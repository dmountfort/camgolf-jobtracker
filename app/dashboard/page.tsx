...
type JobRow={id:string;job_number:number;report_number:string|null;status:Status;service_date:string;updated_at:string;customers:{name:string}|null;sites:{name:string}|null;profiles:{full_name:string}|null;job_vehicles:{count:number}[]};
...
export default function Dashboard(){
 ...
 useEffect(()=>{(async()=>{const supabase=createClient();const {data:{user}}=await supabase.auth.getUser();if(!user){window.location.href="/";return}const {data:profile}=await supabase.from("profiles").select("role").eq("id",user.id).single();if(profile?.role!=="admin"){window.location.replace("/field");return}const {data,error}=await supabase.from("jobs").select("id,job_number,report_number,status,service_date,updated_at,customers(name),sites(name),profiles!jobs_technician_id_fkey(full_name),job_vehicles(count)").is("removed_at",null).order("updated_at",{ascending:false});if(error)setError(error.message);else setJobs((data||[]) as unknown as JobRow[]);setLoading(false)})()},[]);
 const shown=useMemo(()=>jobs.filter(j=>(filter==="all"||(filter==="open"?j.status!=="closed":j.status==="closed"))&&`${j.job_number} ${j.report_number||""} ${j.customers?.name} ${j.sites?.name} ${j.profiles?.full_name}`.toLowerCase().includes(query.toLowerCase())),[jobs,query,filter]);
 ...
 return <main className="app-shell">
  <aside><div className="logo">CG <span>CAM GOLF</span></div><nav><a className="active">Job cards</a><a href="/customers">Customers</a><a>Vehicles</a><a href="/parts">Parts</a><a>Team</a></nav><button className="nav-signout" onClick={signOut}>Sign out</button></aside>
