...
type JobRow={id:string;report_number:number;status:Status;service_date:string;updated_at:string;customers:{name:string}|null;sites:{name:string}|null;profiles:{full_name:string}|null;job_vehicles:{count:number}[]};
...
export default function Dashboard(){
 ...
 const awaiting=jobs.filter(j=>j.status==="completed"||j.status==="under_review").length,open=jobs.filter(j=>j.status!=="closed").length,closed=jobs.filter(j=>j.status==="closed").length;
 ...
 return <main className="app-shell">
  ...
  <div className="stats"><article><span>Awaiting review</span><strong>{awaiting}</strong></article><article><span>Open jobs</span><strong>{open}</strong></article><article><span>Closed</span><strong>{closed}</strong></article></div>
  ...
  {error?<div className="notice error">{error}</div>:loading?<div className="notice">Loading live job cards…</div>:<div className="table"><div className="row heading"><span>Report</span><span>Customer / site</span><span>Date</span><span>Cars</span><span>Status</span></div>{shown.length===0?<div className="empty-table">No job cards match this view.</div>:shown.map(job=><a className="row" href={`/jobs/${job.id}`} key={job.id}><strong>#{String(job.report_number).padStart(5,"0")}</strong><span>{job.customers?.name||"Unknown customer"}<small>{job.sites?.name||"Unknown site"} · {job.profiles?.full_name||"Technician"}</small></span><span>{new Date(job.service_date).toLocaleDateString("en-ZA",{day:"2-digit",month:"short",year:"numeric"})}</span><span>{job.job_vehicles?.[0]?.count||0}</span><span><em className={`pill ${job.status.replace("_","-")}`}>{labels[job.status]}</em></span></a>)}</div>}
  </section>
 </main>
}
