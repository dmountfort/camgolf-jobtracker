...
const labels:Record<Status,string>={draft:"Draft",completed:"In Progress",returned:"In Progress",under_review:"In Progress",closed:"Closed"};
...
export default function Dashboard(){
 ...
 const shown=useMemo(()=>jobs.filter(j=>(filter==="all"||j.status===filter||(filter==="completed"&&["returned","under_review"].includes(j.status)))&&`${j.report_number} ${j.customers?.name} ${j.sites?.name} ${j.profiles?.full_name}`.toLowerCase().includes(query.toLowerCase())),[jobs,query,filter]);
