...
export default function Dashboard(){
 ...
 useEffect(()=>{(async()=>{const supabase=createClient();const {data:{user}}=await supabase.auth.getUser();if(!user){window.location.href="/";return}const {data:profile}=await supabase.from("profiles").select("role").eq("id",user.id).single();if(profile?.role!=="admin"){window.location.replace("/field");return}const {data,error}=await supabase.from("jobs").select("id,report_number,status,service_date,updated_at,customers(name),sites(name),profiles!jobs_technician_id_fkey(full_name),job_vehicles(count)").order("updated_at",{ascending:false});if(error)setError(error.message);else setJobs((data||[]) as unknown as JobRow[]);setLoading(false)})()},[]);
 const shown=useMemo(()=>jobs.filter(j=>(filter==="all"||(filter==="open"?j.status!=="closed":j.status==="closed"))&&`${j.report_number} ${j.customers?.name} ${j.sites?.name} ${j.profiles?.full_name}`.toLowerCase().includes(query.toLowerCase())),[jobs,query,filter]);
